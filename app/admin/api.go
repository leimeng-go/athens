...
func RegisterAPIHandlers(r *mux.Router) {
	// 配置管理API
	r.HandleFunc(Admin+"/api/config", getConfigAPIHandler).Methods("GET")
	r.HandleFunc(Admin+"/api/config", updateConfigAPIHandler).Methods("PUT", "PATCH")
	r.HandleFunc(Admin+"/api/config/reset", resetConfigAPIHandler).Methods("POST")
	r.HandleFunc(Admin+"/api/config/validate", validateConfigAPIHandler).Methods("POST")
