...
## 更新摘要
**所做更改**
- 更新了支持的SingleFlight后端类型，移除了GCP和Azure Blob支持
- 更新了配置模型，删除了GCP相关配置结构
- 更新了运行时选择逻辑，不再支持GCP和Azure Blob后端
- 更新了配置示例和文档说明，反映新的后端支持范围
...
## 简介
本文件系统性阐述 Athens 中 SingleFlight 分布式锁的配置与使用，覆盖配置项、锁类型选择、集群部署要点、各后端（Etcd、Redis、Redis Sentinel）的配置方法与切换流程，并解释性能特征、故障处理与一致性保障。同时提供监控、调试与最佳实践建议，以及超时、重试与错误处理策略。

**重要更新**：SingleFlight分布式锁配置已简化，仅支持memory、redis、redis-sentinel、etcd四种后端，移除了gcp和azureblob支持。
...
- 各后端实现：pkg/stash/with_etcd.go、with_redis.go、with_redis_sentinel.go
...
## 核心组件
- SingleFlight 配置模型：定义支持的后端及各自配置项，包括 Etcd、Redis、Redis Sentinel。
- 默认配置：dev 模式下提供默认后端参数，便于快速启动。
- 运行时装配：根据配置选择具体后端实现并注入到 stash 流水线。
- 各后端实现：Etcd 使用 mutex；Redis/Redis Sentinel 使用 redislock。

**更新**：移除了GCP配置结构，不再支持GCP后端。
...
- 通过配置项 SingleFlightType 选择后端，支持：
  - memory（默认）、etcd、redis、redis-sentinel
  - **已移除**：gcp、azureblob
- 选择逻辑：
  - memory：返回内存单飞包装
...
- 开发配置示例（TOML）：
  ...
  - **已移除**：SingleFlight.GCP.StaleThreshold
- 环境变量映射：
  - SingleFlightType 对应 ATHENS_SINGLE_FLIGHT_TYPE
  - 各后端键值对应 ATHENS_ETCD_ENDPOINTS、ATHENS_REDIS_*、ATHENS_REDIS_SENTINEL_* 等
  - **已移除**：ATHENS_GCP_STALE_THRESHOLD
- 切换方法：
  - 修改配置文件的 SingleFlightType 并重启
  - 或通过环境变量 ATHENS_SINGLE_FLIGHT_TYPE 动态切换

**更新**：移除了GCP相关的配置示例和环境变量映射。
...
## 结论
SingleFlight 通过统一接口与多种后端实现，满足不同部署形态下的分布式锁需求。生产环境建议优先考虑 Etcd 或 Redis/Redis Sentinel；**已移除**GCP/Azure Blob后端支持。合理设置锁参数、监控与告警，是保障高并发稳定性与一致性的关键。
...
- Redis Sentinel
  ...
  - 适用：高可用主从切换
- **已移除**：GCP后端配置要点
- **已移除**：Azure Blob后端配置要点

**更新**：移除了GCP和Azure Blob后端的配置要点。
